    ...
    for k in range (1,6,1):#Runs the iteration for 5 different temperatures
        for i in range(0,6,1): #Runs the iteration for each algorithm
            ...
            if i==0 or i==3:
                ...
                T0=int(500/k) #Decent temperature to get a good amount of iterations on both the exponential and proportional one
            phi=np.copy(phi0)
            t=T0
            ...
            result2=0
            theta=np.copy(phi)
            print("Solving problem 1 using the "+txt[i%3]+" temperature function and the "+txt[int(i/3)+3] +" Phi update function\n")
            ...
            while(t>limit): #will run the annealing algorithm until temp reaches limit
                ...
                result2 = goalFunction(function(dataX, theta)) #gets the value of the goal function using the new phi alternative
...
for b in range(0,5):
    print("\nAverage Min Value for Boltzmann temperature, initial temperature: "+str(1/(b+1)) +" and first Phi update: " +str(avg[0][b]/10))
    print("Average Min Value for Exponential temperature, initial temperature: "+str(500/(b+1)) +"  and first Phi update: " +str(avg[1][b]/10))
    print("Average Min Value for Proportional temperature, initial temperature: "+str(500/(b+1)) +"  and first Phi update: " +str(avg[2][b]/10))
    print("Average Min Value for Boltzmann temperature, initial temperature: "+str(1/(b+1)) +"  and second Phi update: " +str(avg[3][b]/10))
    print("Average Min Value for Exponential temperature, initial temperature: "+str(500/(b+1)) +"  and second Phi update: " +str(avg[4][b]/10))
    print("Average Min Value for Proportional temperature, initial temperature: "+str(500/(b+1)) +"  and second Phi update: " +str(avg[5][b]/10))
